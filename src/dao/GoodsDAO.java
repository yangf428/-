...
public class GoodsDAO {
	...
	public void save(Good good) {
		...
		try {
			...
			String sql = "insert into good(goodname,goodsite,goodexplain,goodtype,fabuid,fabucontact,fabutime,goodtime,goodcampus,goodpicture,goodstate,kind)";
			sql = sql + "values(?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			stat.setTimestamp(8,  st);
			stat.setInt(9, good.getGoodcampus());
			stat.setString(10, good.getPicture());
			stat.setInt(11,good.getGoodState());
			stat.setInt(12, good.getKind());
			stat.execute();
	...
	public void update(Good good) {
		...
		try {
			...
			String sql = "UPDATE good SET goodname=?,goodsite=?,goodexplain=?,goodtype=?,"
					+ "fabucontact=?,goodtime=?,goodcampus=?,goodpicture=? WHERE goodid=?";
			...
			stat.setInt(7, good.getGoodcampus());
	...
	public ArrayList<Good> getGoodsAdFound() {
		...
		try {
			...
			String sql = "select * from good WHERE goodstate=0 AND kind=0";
			stat = conn.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				g.setState(rs.getInt("goodstate"));
				...
				g.setGoodcampus(rs.getInt("goodcampus"));
	...
	public ArrayList<Good> getGoodsAdLost() {
		...
		try {
			...
			String sql = "select * from good WHERE goodstate=0 AND kind=1";
			stat = conn.prepareStatement(sql);
	...
	public ArrayList<Good> getGoodsByGoodstate() {
		...
		try {
			...
			String sql = "select * from good WHERE goodstate=0";
			stat = conn.prepareStatement(sql);
	...
	public ArrayList<Good> getAllGoods() {
		...
		try {
			...
			while (rs.next()) {
				...
				g.setGoodtime(rs.getTimestamp("goodtime"));
				g.setFabucontact(rs.getString("fabucontact"));
				g.setGoodcampus(rs.getInt("goodvalue"));
				g.setPicture(rs.getString("goodpicture"));
	 ...
	 * @return
	 */
	public ArrayList<Good> getGoodByKind(int kind) {

		ArrayList listByKind = new ArrayList();
